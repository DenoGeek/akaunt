...
export function DayCardsSection({
  ...
  return (
    <section>
      <h2 className="text-lg font-medium text-zinc-800 dark:text-zinc-200 mb-1">
        {label}
      </h2>
      <p className="text-sm text-zinc-500 mb-2">{format(date, "EEEE, MMM d")}</p>
      <div className="mb-3 h-1.5 w-full rounded-full bg-zinc-200 dark:bg-zinc-800 overflow-hidden">
        <div
      ...
      {instances.length === 0 && (
        <p className="text-zinc-400 dark:text-zinc-500 text-sm py-2">No tasks.</p>
      )}
    </section>
  );
}
